...
Examples:
  sabbahc main.sabbah         # Compile and link (default output: <filename>{stripped of .sbb}.out)
...
impl CLIInstructions {
    ...
    if output.is_empty() { // Generate output file name from input
        let mut output_file_name = input.clone();
        ...
        match mode {
            ...
            OutputMode::BinaryExecutable => output = format!("{}.out", output_file_name),
        }
    }
